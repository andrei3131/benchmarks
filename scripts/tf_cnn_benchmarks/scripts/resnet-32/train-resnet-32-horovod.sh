...
UPDATE="horovod" # parameter_server, replicated, independent, kungfu
HOROVOD_DEVICE="cpu"
STAGED="False"
...
CHECKPOINT_DIRECTORY="/data/kungfu/checkpoints/andrei-checkpoints/checkpoints-horovod"
...
FLAGS=${FLAGS}" --horovod_device=${HOROVOD_DEVICE}"


# Run
printf "Train ResNet32 v.%s with batch size %4d on %d GPU devices\n" $V $B $G
echo "python3 tf_cnn_benchmarks.py $FLAGS"
mpirun -np 4 -H localhost:4 python3 tf_cnn_benchmarks.py $FLAGS > /data/kungfu/checkpoints/andrei-checkpoints/resnet-32-b-$B-g-$G-horovod.out 2>&1

BASE_DIR=${CHECKPOINT_DIRECTORY}

CHECKPOINT_DIRECTORY="${BASE_DIR}/v-000001"
./scripts/test-resnet-32.sh ${CHECKPOINT_DIRECTORY} > "/data/kungfu/checkpoints/andrei-checkpoints/validation-horovod-output-worker-0" 2>&1


CHECKPOINT_DIRECTORY="${BASE_DIR}/v-000002"
./scripts/test-resnet-32.sh ${CHECKPOINT_DIRECTORY} > "/data/kungfu/checkpoints/andrei-checkpoints/validation-horovod-output-worker-1" 2>&1


CHECKPOINT_DIRECTORY="${BASE_DIR}/v-000002"
./scripts/test-resnet-32.sh ${CHECKPOINT_DIRECTORY} > "/data/kungfu/checkpoints/andrei-checkpoints/validation-horovod-output-worker-2" 2>&1


CHECKPOINT_DIRECTORY="${BASE_DIR}/v-000003"
./scripts/test-resnet-32.sh ${CHECKPOINT_DIRECTORY} > "/data/kungfu/checkpoints/andrei-checkpoints/validation-horovod-output-worker-3" 2>&1

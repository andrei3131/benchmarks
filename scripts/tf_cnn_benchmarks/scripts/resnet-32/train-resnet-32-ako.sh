...
if [ -z "$1" ]
  then
    echo "No arguments supplied for Ako: Need to supply the number of partitions."
	exit 1
fi

# KungFu Synchronization strategy
KUNGFU_STRATEGY="ako" # ako, cpu_all_reduce, nccl_all_reduce
AKO_PARTITIONS=$1   # empty, max_number_of_model_variables
...
CHECKPOINT_DIRECTORY="/data/kungfu/checkpoints/andrei-checkpoints/kungfu-logs-validation/checkpoints-ako-$1"
...
/ab7515/KungFu/bin/kungfu-prun -np=4 -timeout=2000m python3 tf_cnn_benchmarks.py $FLAGS > /data/kungfu/checkpoints/andrei-checkpoints/resnet-32-b-$B-g-$G-ako-$1-partitions.out 2>&1

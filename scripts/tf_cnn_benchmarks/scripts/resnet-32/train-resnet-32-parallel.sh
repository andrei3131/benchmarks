...
CHECKPOINT_DIRECTORY="/data/kungfu/checkpoints/andrei-checkpoints/kungfu-logs-validation/checkpoints-parallel"
...
/ab7515/KungFu/bin/kungfu-prun -np=4 -timeout=2000m python3 tf_cnn_benchmarks.py $FLAGS  # > /data/kungfu/checkpoints/andrei-checkpoints/resnet-32-b-$B-g-$G-parallel.out 2>&1
...
do
    ...
    ./scripts/test-resnet-32.sh ${CHECKPOINT_DIRECTORY} > "/data/kungfu/checkpoints/andrei-checkpoints/kungfu-logs-validation/validation-parallel-worker-${worker}" 2>&1
done
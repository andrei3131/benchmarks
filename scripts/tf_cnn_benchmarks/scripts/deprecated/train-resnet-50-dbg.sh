...
# TF record data
# DATADIR="/mnt/nfs/users/piwatcha/my-tensorflow/data/imagenet/records-dbg"
DATADIR="/mnt/nfs/users/piwatcha/my-tensorflow/models/research/inception/inception/data/dataset/records"
# On platypus, the records exist under "/data/tf/imagenet/records"
...
FLAGS=${FLAGS}" --batchnorm_persistent=True" # Default true
FLAGS=${FLAGS}" --use_tf_layers=True" # Default true
FLAGS=${FLAGS}" --winograd_nonfused=True" # Default true

# Run.
#
python tf_cnn_benchmarks.py $FLAGS > results/debug/b-${my_b}-g-${my_g}-debug.out 2>&1

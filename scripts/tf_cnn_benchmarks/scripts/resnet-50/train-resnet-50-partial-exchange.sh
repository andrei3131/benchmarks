...
CHECKPOINT_DIRECTORY="/cache/kungfu-logs-${KUNGFU_STRATEGY}/checkpoints-${KUNGFU_STRATEGY}-fraction-$1"
...
# Run
printf "Train ResNet50 v.%s with batch size %4d on %d GPU devices\n" $V $B $G
printf "Number of epochs is ${EPOCHS}"
/ab7515/KungFu/bin/kungfu-prun -np=4 -timeout=2000m python3 tf_cnn_benchmarks.py $FLAGS

...
CHECKPOINT_DIRECTORY="/data/kungfu/checkpoints/andrei-checkpoints/resnet-50/checkpoints-horovod"
...
mpirun -np 4 -H localhost:4 python3 tf_cnn_benchmarks.py $FLAGS > /data/kungfu/checkpoints/andrei-checkpoints/resnet-50/resnet-50-b-$B-g-$G-horovod.out 2>&1

BASE_DIR=${CHECKPOINT_DIRECTORY}

for version in "v-000001" "v-000002" "v-000002" "v-000003"
do
	for worker in 0 1 2 3
	do
		./scripts/test-resnet-50.sh "${BASE_DIR}/${version}" > "/data/kungfu/checkpoints/andrei-checkpoints/resnet-50/validation-horovod-output-worker-${worker}" 2>&1
	done
done

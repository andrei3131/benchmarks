...
class ConvNetBuilder(object):
    ...
    def cbow_conv(self,
        ...
        name = 'conv' + str(self.counts['conv'])
        self.counts['conv'] += 1
        input_layer = self.top_layer if inputs is None else inputs
    ...
    def cbow_dropout(self, inputs=None, rate=0.5, training=None):
        ...
        with tf.variable_scope(name):
            y = tf.layers.dropout(inputs=input_layer, rate=rate, training=training)
    ...
    def cbow_dense(self,
        ...
        name = 'dense' + str(self.counts['dense'])
        self.counts['dense'] += 1
        input_layer = self.top_layer if inputs is None else inputs
    ...
    def cbow_mpool(self, pool_size, strides, inputs=None, padding="valid", data_format="channels_first"):
